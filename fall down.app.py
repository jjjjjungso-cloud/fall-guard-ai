...
# --------------------------------------------------------------------------------
st.set_page_config(
    page_title="SNUH Ward EMR - AI & Smart Charting",
    page_icon="🏥",
    layout="wide",
...
# --------------------------------------------------------------------------------
# 2. [핵심] AI 모델 및 변수 리스트 로딩 (선생님 코드 반영)
# --------------------------------------------------------------------------------
@st.cache_resource
def load_ai_model():
    try:
        ...
        # 2. 변수 리스트 파일 로딩 (CSV)
        df_features = pd.read_csv('rf_model_feature_columns.csv')
        feature_columns = df_features['feature'].tolist()
        
        return model, feature_columns
    except Exception as e:
        # 파일이 없을 경우를 대비한 예외처리 (앱이 꺼지지 않게)
        return None, []

model, feature_cols = load_ai_model()

# --------------------------------------------------------------------------------
# 3. [핵심] 예측 함수 (환자 정보 -> AI 입력 변환)
# --------------------------------------------------------------------------------
def predict_fall_risk(pt_info):
    if model is None: return 0  # 모델 없으면 0점
    ...
    # 3. 데이터 매핑 (범주형 - 예시)
    # 실제로는 학습 데이터의 컬럼명과 정확히 일치시켜야 함
    if pt_info.get('gender') == 'M': input_data['성별'] = 1
    
    # 4. DataFrame 변환 및 예측
    try:
        input_df = pd.DataFrame([input_data])
        input_df = input_df[feature_cols] # 순서 강제 정렬
...
# --------------------------------------------------------------------------------
# 5. 데이터 초기화 (환자 DB + AI 입력용 상세 데이터 포함)
# --------------------------------------------------------------------------------
...
# [환자 DB] AI 모델에 넣을 V/S, 검사수치(albumin) 등을 포함
PATIENTS_DB = [
    {
...
with col_sidebar:
    ...
    for idx, p in enumerate(PATIENTS_DB):
        ...
        risk_dot = "🔴" if predict_fall_risk(p) >= 60 else "" # 리스트에도 위험도 살짝 표시
